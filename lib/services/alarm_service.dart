...
class AlarmService {
  ...
  Future<List<AlarmModel>> getAllAlarms() async {
    try {
      ...
      return alarmsData.map((data) => AlarmModel.fromMap(data)).toList();
    } catch (e) {
  ...
  /// Save an alarm (updates in place when the id already exists)
  ...
  Future<bool> _persist(List<AlarmModel> alarms) async {
    final prefs = await SharedPreferences.getInstance();
